<x-app-layout>
    <x-slot name="header">{{ isset($riwayat) ? 'Edit Riwayat Budidaya' : 'Tambah Riwayat Budidaya' }}</x-slot>
    <x-slot name="description">Pastikan data riwayat budidaya yang diinputkan sudah benar.</x-slot>

    <div class="max-w-3xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
        
        <form action="{{ isset($riwayat) ? route('riwayat-budidaya.update', $riwayat->id) : route('riwayat-budidaya.store') }}" method="POST" class="space-y-6">
            @csrf
            @if(isset($riwayat))
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $riwayat->tanggal ?? date('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select name="jenis_kegiatan" required class="w-full border-slate-200 rounded-xl focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach(['Tanggal Tanam', 'Pemupukan Pertama', 'Penyemprotan', 'Penyiangan', 'Pemupukan Kedua', 'Perawatan', 'Terakhir Panen'] as $jenis)
                            <option value="{{ $jenis }}" {{ old('jenis_kegiatan', $riwayat->jenis_kegiatan ?? '') == $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                        @endforeach
                    </select>
                        ...
                        @forelse($pekerjas as $pk)
                            <label class="flex items-center gap-2 cursor-pointer hover:bg-slate-50 rounded-lg px-2 py-1.5">
                                <input type="checkbox" name="pekerja_ids[]" value="{{ $pk->id }}"
                                    ...
                                    {{ in_array($pk->id, old('pekerja_ids', isset($riwayat) ? $riwayat->pekerjas->pluck('id')->toArray() : [])) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" class="w-full border-slate-200 rounded-xl focus:ring-emerald-500 focus:border-emerald-500" placeholder="Contoh: Pemupukan pertama NPK 15-15-15...">{{ old('keterangan', $riwayat->keterangan ?? '') }}</textarea>
            ...
            <div class="flex items-center gap-4 pt-4 border-t border-slate-100">
                <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-medium transition-colors">
                    {{ isset($riwayat) ? 'Simpan Perubahan' : 'Tambah Data' }}
                </button>
                <a href="{{ route('riwayat-budidaya.index') }}" class="text-slate-500 hover:text-slate-700 font-medium px-4 py-2">
                    Batal
                </a>
            </div>
        </form>
