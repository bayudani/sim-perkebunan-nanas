<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Riwayat Budidaya</h1>
            <p class="text-sm text-slate-500 mt-1">Riwayat Kegiatan Budidaya Perkebunan Nanas</p>
        </div>
        
        <!-- Tombol Tambah Riwayat -->
        @if(auth()->user()->role === 'admin')
        <a href="{{ route('riwayat-budidaya.create') }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-medium transition-colors shadow-sm">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-5 w-full shadow-sm">
            <p class="text-sm font-semibold text-emerald-700 mb-1">Total Riwayat Budidaya</p>
            <h3 class="text-2xl font-bold text-slate-800">{{ $riwayats->count() }} <span class="text-lg text-slate-500 font-medium">Kegiatan</span></h3>
            <p class="text-xs font-medium text-slate-500 mt-1">{{ $riwayats->unique('jenis_kegiatan')->count() }} Jenis kegiatan berbeda</p>
        </div>
                ...
                <tbody class="text-slate-700 text-sm">
                    ...
                    <tr class="group drop-shadow-sm hover:drop-shadow-md transition-all">
                        ...
                        <td class="bg-white p-4 rounded-r-2xl group-hover:bg-emerald-50/30 transition-colors">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('riwayat-budidaya.edit', $item->id) }}" class="px-4 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-full transition-colors font-semibold text-xs">
                                    Edit
                                </a>
                                <form action="{{ route('riwayat-budidaya.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="{{ auth()->user()->role === 'admin' ? 7 : 6 }}" class="bg-white p-8 text-center text-slate-400 rounded-2xl shadow-sm">
                            Belum ada riwayat budidaya tercatat.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
